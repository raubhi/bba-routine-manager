...
if course_file:
    ...
    # --- 3. ADVANCED BATCH & GLOBAL SETTINGS ---
    st.subheader("3. Advanced Scheduling & Batch Routing Controls")
    c_set1, c_set2 = st.columns(2)
    
    with c_set1:
        st.write("**Sunday / Monday Batch Force Routing**")
        sun_mon_batches = st.multiselect(
            "Select Batches that must prioritize Sunday & Monday:",
            ["20th Batch", "19th Batch", "18th Batch", "17th Batch", "16th Batch", "15th Batch", "14th Batch"],
            default=["15th Batch", "14th Batch"]
        )
        include_majors = st.toggle("Include Major Courses", value=False)
        
    with c_set2:
        st.write("**TBA Management**")
    ...
    if st.button("🚀 Generate Full Multi-Batch Master Routine", type="primary", use_container_width=True):
        with st.spinner("Executing OR-Tools CP-SAT multi-batch constraint matrix..."):
            ...
            # CP-SAT Solver Core with Soft/Hard Preference for Sunday/Monday Batches
            ...
            # Objective: Prioritize Sunday/Monday for selected senior batches
            objective_terms = []
            for i, task in enumerate(master_tasks):
                if any(b in task['Batch'] for b in sun_mon_batches):
                    for d in ["Sunday", "Monday"]:
                        for s in time_slots:
                            objective_terms.append(x[(i, d, s)])
            if objective_terms:
